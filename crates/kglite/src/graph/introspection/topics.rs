...
const CYPHER_TOPIC_LIST: &str = "MATCH, WHERE, FILTER, RETURN, FINISH, WITH, HAVING, ORDER BY, OFFSET, UNWIND, UNION, \
    ...
    type_domain_violation, type_range_violation, parallel_edges, ontology, node_semantic, relationship_semantic";

/// The node `semantic` functions group, shared with the direct `node_semantic`
/// topic so the two cannot drift apart.
const NODE_SEMANTIC_GROUP: &str = "text_score(n, 'col', 'query'|[0.1,0.2,...] [, metric] [, {exact:true}]) — similarity score; exact:true bypasses HNSW, otherwise a compatible top-k may use it; result diagnostics report actual_mode and fallback_reason (also with PROFILE), while EXPLAIN reports requested policy only; a list query is scored as your query vector, a string query is embedded via set_embedder() (metrics: 'cosine', 'poincare', 'dot_product', 'euclidean'); embedding_norm(n, 'col_emb') — L2 norm of embedding vector (hierarchy depth in Poincaré space, 0=root, ~1=leaf); embedding(n, 'col_emb') — the stored vector as a list of floats (null when the node has none), e.g. vector_score(m, 'col_emb', embedding(n, 'col_emb')) for node-to-node similarity; a node with no vector scores null, and openCypher sorts null first, so ORDER BY score DESC LIMIT k returns unembedded nodes first — a type with unembedded members is still answered from its store: the null-scored nodes first in type order, then the store's ranking; row_coverage is reported only when all k rows are null or the store's order differs from the type's. Add WHERE vector_score(n, 'col_emb', $v) IS NOT NULL (or the text_score form) to the MATCH to leave them out; it is served from the store at the store procedure's cost; vector_search/search_text never return them";
...
const RELATIONSHIP_SEMANTIC_GROUP: &str = "vector_score(r, 'col_emb', $v), text_score(r, 'col', 'query'|[...] [, metric]) and embedding_norm(r, 'col_emb') score a bound relationship against its relationship embedding store, exactly, row by row; embedding(r, 'col_emb') returns the stored vector itself (null when it has none), so vector_score(r2, 'col_emb', embedding(r1, 'col_emb')) scores one relationship against another. The top-k shape RETURN … vector_score(r, …) AS s ORDER BY s DESC LIMIT k (or text_score) is served from the store, as for nodes: a plain single-type pattern whose every relationship is embedded goes straight to the store, any other shape scores its rows; WITH r, vector_score(r, …) AS s ORDER BY s DESC LIMIT k RETURN startNode(r)… is served the same way, and an undirected (a)-[r:T]-(b) uses the index, returning each relationship once per orientation; an alternation [r:A|B] or untyped [r] merges per-store answers when every type in play carries the store (a type without it raises the scalar's error); HNSW answers (approximate) when an index is online, pass {exact:true} as the final argument to force exact; the answer equals the unfused query's up to ties (which rows tied at the cut make the top k can differ between HNSW and exact), and result diagnostics report the route. A relationship with no vector scores null and openCypher sorts null first, so a DESC top-k returns unembedded relationships first, and a type whose relationships are not all embedded is answered by row scan (row_coverage) — add WHERE vector_score(r, 'col_emb', $v) IS NOT NULL to the MATCH, which is served from the store at the store procedure's cost; db.relationship_embeddings.query never returns them. Stores are per (relationship type, text column). To rank a whole relationship store without a pattern, CALL db.relationship_embeddings.query({type:'T', text_column:'col', vector:$v | text:'query', top_k:10}) YIELD relationship, score, search_method, type — HNSW once db.relationship_embeddings.build_index has run; types:['A','B'] (or neither type nor types: every store for text_column) ranks several stores merged into one top-k.";
...
pub(super) fn write_cypher_topics(
    ...
    for topic in topics {
        ...
        match key.as_str() {
            ...
            "NODE_SEMANTIC" | "RELATIONSHIP_SEMANTIC" => write_topic_semantic(xml, &key),
            _ => {
                return Err(format!(
...
pub(super) fn write_topic_functions(xml: &mut String) {
    ...
    xml.push_str("    <group name=\"graph\">size(list), length(path), id(node) — the node's own id FIELD (same as n.id), not an engine-assigned identifier and not a positional index, labels(node), type(rel), coalesce(expr,...) — first non-null, range(start,end[,step]) — checked inclusive list subject to the max_work_units budget and a 256 MiB materialization ceiling, keys(node)/keys(rel)/keys(map) — sorted key names, properties(node)/properties(rel) — full property map (a relationship's keys and map also carry its type), start_node(rel)/end_node(rel) — endpoints</group>\n");
    ...
    xml.push_str("    <group name=\"lexical\">text_bm25(n, 'prop', 'query text') — BM25 relevance of the node's indexed text; needs build_text_index(node_type, property) first (error if absent). text_bm25(r, 'prop', 'query text') does the same for a relationship binding or value (collect(r)[0], UNWIND, the relationship column of db.relationship_embeddings.query) over an index built with CALL db.relationship_text_index.build({type, text_column}). 0.0 = indexed but shares no word with the query, null = the index has no document for that row. Composes with WHERE and ORDER BY score DESC LIMIT k</group>\n");
    ...
    xml.push_str(&format!(
        "    <group name=\"semantic\">{NODE_SEMANTIC_GROUP}</group>\n"
    ));
    xml.push_str(&format!(
...
pub(super) fn write_topic_explain(xml: &mut String) {
    ...
    xml.push_str("    </examples>\n");
    xml.push_str("    <notes>Cardinality estimates use type_indices counts. Fused optimizations shown as single steps. Each variable-length edge adds an Expand row after its Match row, with estimated_rows null (no cardinality model covers var-length expansion). A ClosureProbe :Super (Member, ...) row after a Match row means that pattern's materialized-ontology closure is answered from the member types' indexes instead of a scan; no row means no probe (open label, unindexed member, or nothing materialized). estimated_rows is null on it, and a parameter-supplied value leaves the row off because the plan renders before the parameter binds.</notes>\n");
    xml.push_str("  </EXPLAIN>\n");
}
...
pub(super) fn write_fluent_overview(xml: &mut String, surface: DescribeSurface) {
    ...
    xml.push_str("    <method sig=\"create_relationships(connection_type)\">Materialise direct edges from traversal chain.</method>\n");
    xml.push_str("  </group>\n");
    ...
    xml.push_str("    <method sig=\"search_text(text_column, query, top_k=10, metric=None, to_df=False, returning=None, exact=False, *, entity='node', types=None, relationship_keys=None)\">Semantic text search (auto-embeds query). Note the argument order: column first, then the query string. entity='relationship' ranks relationship stores.</method>\n");
    xml.push_str("    <method sig=\"vector_search(text_column, query_vector, top_k=10, metric=None, to_df=False, returning=None, exact=False, *, entity='node', types=None, relationship_keys=None)\">Search with a pre-computed query vector. entity='relationship' ranks relationship stores.</method>\n");
    xml.push_str("  </group>\n");
    ...
    xml.push_str("    <method sig=\"add_relationships(data, connection_type, source_type, source_id_field, target_type, target_id_field)\">Load edges from DataFrame.</method>\n");
    ...
    xml.push_str("    <method sig=\"set_recipe(recipe, name, description, cypher, parameters=None, recipe_description=None, tool=None)\">Store a query. Refused unless the Cypher is read-only and the parameter schema matches its $parameters exactly. tool= names an MCP tool an agent host serves the query under directly.</method>\n");
...
pub(super) fn write_fluent_topic_traversal(xml: &mut String) {
    ...
    xml.push_str("      <m sig=\"create_relationships(connection_type)\">Materialise direct edges from a traversal chain.</m>\n");
    xml.push_str("    </methods>\n");
    xml.push_str("    <examples>\n");
...
pub(super) fn write_fluent_topic_vectors(xml: &mut String) {
    ...
    xml.push_str("      <m sig=\"embed_texts(node_type, text_column, batch_size=256, show_progress=True, mode=None, *, entity='node', metric=None)\">Compute and store embeddings for a text column. embed_node_texts / embed_relationship_texts are the routes; entity='relationship' picks the second.</m>\n");
    xml.push_str("      <m sig=\"set_embeddings(node_type, text_column, embeddings, metric=None, *, entity='node', relationship_keys=None)\">Provide pre-computed embeddings {id: vector} — replaces the store. text_column names the source text column, which must exist on the type. set_node_embeddings is the node route; entity='relationship' routes to set_relationship_embeddings.</m>\n");
    xml.push_str("      <m sig=\"add_embeddings(node_type, text_column, embeddings, metric=None, *, entity='node', relationship_keys=None)\">Same, upserting into the existing store so batches coexist (add_node_embeddings; entity='relationship' routes to add_relationship_embeddings). Call save() to persist either.</m>\n");
    xml.push_str("      <m sig=\"search_text(text_column, query, top_k=10, metric=None, to_df=False, returning=None, exact=False, *, entity='node', types=None, relationship_keys=None)\">Semantic search — auto-embeds the query string. Column first, then the query. Ranks the selection's nodes (node_search_text); entity='relationship' routes to relationship_search_text, which ranks every relationship type with a text_column store (or types=[...]) merged into one top-k.</m>\n");
    xml.push_str("      <m sig=\"vector_search(text_column, query_vector, top_k=10, metric=None, to_df=False, returning=None, exact=False, *, entity='node', types=None, relationship_keys=None)\">Search with an explicit query vector (node_vector_search; entity='relationship' routes to relationship_vector_search). A relationship hit is a relationship_embeddings row without the vector, plus relationship_type and score: {source, target, source_type, target_type, key, relationship_type, score}. One search runs per relationship type.</m>\n");
    xml.push_str("      <m sig=\"build_vector_index(node_type, text_column, m=None, ef_construction=None, ef_search=None, metric=None, auto_refresh_limit=None, *, entity='node')\">Build an HNSW index so search scales on large stores (opt-in; auto-used; exact=True bypasses). Later vector writes are folded in at query entry while the delta stays under auto_refresh_limit; deleting an embedded node drops it, and a vacuum() that compacts drops every vector index in the graph.</m>\n");
    xml.push_str("      <m sig=\"refresh_vector_index(node_type, text_column, *, entity='node') / drop_vector_index(node_type, text_column, *, entity='node') / has_vector_index(node_type, text_column, *, entity='node')\">Fold outstanding vectors in now (refuses when no index is built — it never builds one), remove the index (the vectors stay), or probe one. SHOW INDEXES reports stale/delta/unembedded. All four index methods take entity=; build_/refresh_/drop_/has_relationship_vector_index(relationship_type, text_column) are the relationship routes.</m>\n");
    xml.push_str("      <m sig=\"embeddings(node_type_or_text_column, text_column, *, entity='node', relationship_keys=None) / embedding(node_type, text_column, node_id, *, entity='node', relationship_keys=None) / relationship_embeddings(relationship_type, text_column, *, relationship_keys=None) / relationship_embedding(relationship_type, text_column, address, *, relationship_keys=None)\">Read stored vectors back (node_embeddings / relationship_embeddings are the routes of embeddings(entity=...)): a node store by node id, or a relationship store as rows {source, target, source_type, target_type, key, vector} (an edge list plus edge features). relationship_embedding reads one relationship's vector by an address shaped like a writer's dict key ((source_id, target_id), plus the key, or with endpoint types). Every reader here refuses a store that does not exist, by name. In Cypher, embedding(x, 'col_emb') returns one node's or relationship's vector.</m>\n");
    xml.push_str("      <m sig=\"remove_embeddings(node_type, text_column, *, entity='node') / embedding_dim(node_type, text_column, *, entity='node')\">Remove a whole store (remove_node_embeddings / remove_relationship_embeddings; refuses a store that does not exist, naming the one you probably meant), or read a store's dimension (node_embedding_dim / relationship_embedding_dim; None when absent).</m>\n");
    xml.push_str("      <m sig=\"set_relationship_embeddings(relationship_type, text_column, embeddings, *, relationship_keys=None, metric=None) / add_relationship_embeddings(relationship_type, text_column, embeddings, *, relationship_keys=None, metric=None) / embed_relationship_texts(relationship_type, text_column, batch_size=256, show_progress=True, mode=None, *, metric=None)\">Bulk-write a relationship store: set_ replaces it and add_ upserts into it (as set_embeddings / add_embeddings for nodes), with vectors keyed by (source_id, target_id) — or (source_type, source_id, target_type, target_id), plus the key for a parallel-group member — or the rows relationship_embeddings returns; or embed every relationship of the type with the registered model, as embed_texts does for nodes. db.relationship_embeddings.set / .embed are the in-query routes.</m>\n");
    xml.push_str("    </methods>\n");
    xml.push_str("    <examples>\n");
...
pub(super) fn write_fluent_topic_loading(xml: &mut String) {
    ...
    xml.push_str(
        "    <desc>Load nodes and relationships from DataFrames or blueprint files. The connection-named loaders (add_connections, replace_connections, add_connections_bulk, create_connections) are permanent pointers to these relationship-named ones.</desc>\n",
    );
    xml.push_str("    <methods>\n");
    xml.push_str("      <m sig=\"add_nodes(data, node_type, unique_id_field, node_title_field=None, columns=None, conflict_handling=None, column_types=None, timeseries=None, labels=None, git_sha=None, modified_by=None, on_invalid='warn')\">Load nodes. conflict_handling: 'update'|'replace'|'skip'|'preserve'|'sum'. on_invalid: 'warn' (default, skip unusable rows and warn), 'error' (refuse the whole call, nothing written), 'skip' (silent). Provenance is stamped on auto_timestamp types. A type's unique_id_field/node_title_field is fixed once it has nodes: naming a different one raises and writes nothing.</m>\n");
    xml.push_str("      <m sig=\"add_relationships(data, connection_type, source_type, source_id_field, target_type, target_id_field, columns=None, skip_columns=None, conflict_handling=None, query=None, extra_properties=None, git_sha=None, modified_by=None, on_invalid='warn')\">Load edges from DataFrame or a read query. on_invalid controls rows with a null endpoint id, as in add_nodes. Provenance is stamped on auto_timestamp edge types.</m>\n");
    xml.push_str("      <m sig=\"replace_relationships(data, connection_type, source_type, source_id_field, target_type, target_id_field, ...)\">Atomic edge upsert: prune each source node's existing connection_type edges, then add the input's. Same args as add_relationships; use to re-sync a derived edge set idempotently.</m>\n");
    ...
    xml.push_str(
        "      <m sig=\"add_relationships_bulk(connections, git_sha=None, modified_by=None)\">Bulk load multiple relationship types with optional provenance.</m>\n",
    );
    ...
    xml.push_str("      <ex desc=\"edges\">graph.add_relationships(df, 'WORKS_AT', 'Person', 'person_id', 'Company', 'company_id')</ex>\n");
    xml.push_str("      <ex desc=\"edges from query\">graph.add_relationships(None, 'ENCLOSES', 'Play', 'play_id', 'Area', 'area_id', query='MATCH (p:Play), (a:Area) WHERE contains(p, a) RETURN DISTINCT p.id AS play_id, a.id AS area_id')</ex>\n");
...
pub(super) fn write_fluent_topic_indexes(xml: &mut String) {
    ...
    xml.push_str("      <m sig=\"build_text_index(node_type, property)\">BM25 lexical index over text or lists of strings/nulls; lists form one document. Opt-in and explicit: rebuild by calling again after writes; deletes prune, vacuum drops.</m>\n");
...
pub(super) fn write_fluent_topic_schema(xml: &mut String) {
    ...
    xml.push_str(
        "      <m sig=\"relationship_types()\">All relationship types with counts and endpoints (connection_types() is a pointer to it).</m>\n",
    );
    xml.push_str(
...
pub(super) fn write_cypher_overview(xml: &mut String, surface: DescribeSurface) {
    ...
    xml.push_str("    <clause name=\"DROP INDEX\">DROP INDEX &lt;canonical-name&gt; [IF EXISTS], e.g. DROP INDEX Person.email — or the descriptor form DROP INDEX FOR (n:Label) ON (n.prop). Removes every structure registered under that name. Any name SHOW INDEXES prints is accepted unquoted, including a relationship index's qualified form (DROP INDEX relationship:SUPPORTS.evidence), which drops every relationship structure under that name: the HNSW accelerator (the vectors stay) and the BM25 text index. IF EXISTS is a no-op only when nothing carries the name.</clause>\n");
    xml.push_str("    <clause name=\"SHOW INDEXES\">List installed indexes as a read: columns name, type (PROPERTY|RANGE|FULLTEXT|VECTOR), entityType (NODE, or RELATIONSHIP for a relationship vector or BM25 text index, named relationship:TYPE.property), labelsOrTypes, properties, state, stale, delta, unembedded. stale/delta are null on PROPERTY and RANGE rows; on FULLTEXT (BM25) and VECTOR rows they report whether the index is behind the graph and by how many documents or vectors. unembedded is VECTOR-only. Same rows as CALL db.indexes(); use that form when you need YIELD or WHERE.</clause>\n");
    ...
    xml.push_str("    <clause name=\"SHOW ONTOLOGY\">List the declared semantic layer as a read: one row per class and relationship, including required_properties, property_types and enforcement; classes also expose is_a/abstract, relationships domain/range/exempt. Zero rows when no ontology is declared. Declared with define_ontology(); audited with CALL ontology_audit().</clause>\n");
    ...
    xml.push_str("    <proc name=\"db.node_embeddings.*\" yields=\"per procedure; query: node, score, search_method, type\">Node embedding stores through Cypher — the relationship procedures below with nodes in place of relationships, over the same stores set_embeddings / embed_texts / build_vector_index write. db.node_embeddings.set({type, text_column, entries:[{node:n, vector:[...]}], metric?}) YIELD stored, dimension upserts (add_embeddings' path); db.node_embeddings.embed({type | types:[...], text_column, nodes, mode?, batch_size?, metric?}) YIELD embedded, skipped, dimension, model embeds the listed nodes with the registered embedder; list, remove({type, text_column, nodes}), drop, build_index, refresh_index, drop_index take the relationship twins' parameters and yield their columns; db.node_embeddings.query({type | types:[...], text_column, vector:[...] | text:'query', top_k?, exact?, metric?}) YIELD node, score, search_method, type ranks whole stores merged into one top-k (stores declaring different metrics refuse the merge unless metric is passed). Writes roll back with a failing statement.</proc>\n");
    xml.push_str("    <proc name=\"db.node_text_index.*\" yields=\"build: indexed, skipped, terms; refresh: refreshed; drop: dropped; list: entity, type, text_column, documents, terms, skipped, index_state, delta, auto_refresh_limit\">The node BM25 index lifecycle in Cypher, over build_text_index's path: build({type, text_column, auto_refresh_limit?}), refresh({type, text_column}), drop({type, text_column}), list({type?, text_column?}). Score with text_bm25(n, 'property', 'query').</proc>\n");
    xml.push_str("    <proc name=\"db.embeddings.* / db.text_index.*\" yields=\"as the procedure routed to\">Routers: the same operations with an entity:'node' | 'relationship' key (a string literal; default 'node') that runs db.node_embeddings.* / db.node_text_index.* or db.relationship_embeddings.* / db.relationship_text_index.*. A parameter only the other entity takes is refused naming it.</proc>\n");
    xml.push_str("    <proc name=\"db.relationship_embeddings.*\" yields=\"per procedure; query: relationship, score, search_method, type\">Relationship embedding stores, keyed by (relationship type, text column). db.relationship_embeddings.set({type, text_column, entries:[{relationship:r, vector:[...]}], metric?}) YIELD stored, dimension writes vectors (stored = vectors the store holds after the call, not the number written; set and embed refuse a text_column no relationship of the type carries); db.relationship_embeddings.embed({type | types:[...], text_column, relationships, mode?, batch_size?, metric?}) YIELD embedded, skipped, dimension, model generates them with the registered embedder (types:[...] runs one pass per listed type and yields one row: embedded and skipped summed, dimension and model shared or null); from Python, set_/add_relationship_embeddings and embed_relationship_texts are the bulk routes; db.relationship_embeddings.list({type?, text_column?}) YIELD entity, type, text_column, store, dimension, count, metric, model, index_state, delta, unembedded (delta = stored vectors the index does not hold, so delta equals count when no index is built); db.relationship_embeddings.remove({type, text_column, relationships}) and db.relationship_embeddings.drop({type, text_column}) delete vectors or the whole store; db.relationship_embeddings.build_index({type, text_column, m?, ef_construction?, ef_search?, metric?, auto_refresh_limit?}), db.relationship_embeddings.refresh_index({type, text_column}) and db.relationship_embeddings.drop_index({type, text_column}) manage the HNSW accelerator, which .kgl saves keep (refresh_index refuses when no index is built; SET and CREATE keep the index, while deleting an embedded relationship — DELETE r, or DETACH DELETE of either endpoint — drops it to index_state none until build_index runs again, as a node delete drops the node index; a vacuum() that compacts drops every vector index in the graph, node and relationship, including types that saw no delete); db.relationship_embeddings.query({type | types:[...], text_column, vector:[...] | text:'query', top_k?, exact?, metric?}) YIELD relationship, score, search_method, type ranks the whole store — or with types:[...], or with neither type nor types (every store for text_column), several stores merged into one top-k ordered by score, then type, then slot; stores declaring different metrics refuse the merge unless metric is passed (search_method is per row: hnsw only when that store's index served the query, else exact — exact:true, a stale index that cannot catch up, or a metric the index cannot serve all answer exact; text: is embedded once with the registered embedder). Inside MATCH, vector_score(r, 'col_emb', $v), text_score(r, 'col', ...) and embedding_norm(r, 'col_emb') score a bound relationship per row; ORDER BY vector_score(r, …) DESC LIMIT k is served from the store (HNSW when an index is online, {exact:true} forces exact) — see the relationship_semantic functions group.</proc>\n");
    xml.push_str("    <proc name=\"db.relationship_text_index.*\" yields=\"build: indexed, skipped, terms; refresh: refreshed; drop: dropped; list: entity, type, text_column, documents, terms, skipped, index_state, delta, auto_refresh_limit\">BM25 text indexes over one relationship type's property, the relationship twin of build_text_index. db.relationship_text_index.build({type, text_column, auto_refresh_limit?}) builds or replaces one; db.relationship_text_index.refresh({type, text_column}) folds in every change since; db.relationship_text_index.drop({type, text_column}) removes it (dropped false when absent); db.relationship_text_index.list({type?, text_column?}) reads freshness. Score with text_bm25(r, 'property', 'query'). Writes (SET, REMOVE, CREATE, MERGE) are folded in at the next query within auto_refresh_limit, a deleted relationship's document is removed at the delete, and a failed statement's build, drop or catch-up is undone. Saved in .kgl; memory and mapped storage only (disk refuses); not in the write-ahead log.</proc>\n");
    xml.push_str("  </procedures>\n");
...
/// The two vector-retrieval topics, node and relationship, as one arm of the
/// topic dispatch.
fn write_topic_semantic(xml: &mut String, key: &str) {
    if key == "NODE_SEMANTIC" {
        write_topic_node_semantic(xml);
    } else {
        write_topic_relationship_semantic(xml);
    }
}

/// Node vector retrieval as one topic — the node twin of
/// [`write_topic_relationship_semantic`].
pub(super) fn write_topic_node_semantic(xml: &mut String) {
    xml.push_str("  <topic name=\"node_semantic\">\n");
    xml.push_str(&format!("    <summary>{NODE_SEMANTIC_GROUP}</summary>\n"));
    xml.push_str("    <usage>An embedding store belongs to one node type and one text column (describe() lists it under the type as an embeddings element with text_col, dim and count, and index=\"hnsw\" once an index is built). Write vectors from Python with set_embeddings / add_embeddings / embed_texts, or in a query with db.node_embeddings.set or .embed; index with build_vector_index or db.node_embeddings.build_index. The top-k shape MATCH (n:T) RETURN n, vector_score(n, 'col_emb', $v) AS s ORDER BY s DESC LIMIT k (or text_score) is served from the store, through HNSW once an index is online ({exact:true} as the final argument forces exact). To rank whole stores without a pattern, CALL db.node_embeddings.query({type:'T' | types:['A','B'], text_column:'col', vector:$v | text:'query', top_k:10}) YIELD node, score, search_method, type — several types merge into one top-k ordered by score, then type, then slot, one search per type. From Python, vector_search / search_text rank the current selection. db.embeddings.* routes to these procedures by default and to db.relationship_embeddings.* with entity:'relationship'.</usage>\n");
    xml.push_str("    <caveat>vector_score and embedding_norm take the store name ('col_emb'); text_score takes the text column ('col'). A node with no vector scores null, and null sorts first under DESC. Deleting an embedded node drops that store's HNSW index to none until it is built again, and a vacuum() that compacts drops every vector index in the graph; refresh_index refuses while there is none.</caveat>\n");
    xml.push_str("    <ex desc=\"store procedure\">CALL db.node_embeddings.query({type:'Doc', text_column:'summary', text:'graph databases', top_k:5}) YIELD node, score RETURN node.title, score</ex>\n");
    xml.push_str("    <ex desc=\"fused top-k\">MATCH (d:Doc) RETURN d.title, vector_score(d, 'summary_emb', $v) AS score ORDER BY score DESC LIMIT 5</ex>\n");
    xml.push_str("  </topic>\n");
}
...
pub(super) fn write_topic_relationship_semantic(xml: &mut String) {
    ...
    xml.push_str("    <usage>An embedding store belongs to one relationship type and one text column (describe() shows it on that type's conn line as embeddings=\"col(dim=D,count=N)\", with ,hnsw once an index is built). Write vectors with db.relationship_embeddings.set or .embed in a query, or in bulk from Python with set_/add_relationship_embeddings / embed_relationship_texts; index with db.relationship_embeddings.build_index. To rank across relationship types, either CALL db.relationship_embeddings.query({types:['A','B'], text_column:'col', ...}) — or leave out type and types to rank every store for text_column — which merges the stores into one top-k ordered by score, then type, then slot (each row yields type and its own search_method); or MATCH ()-[r:A|B]->() (or untyped ()-[r]->()) RETURN r, vector_score(r, 'col_emb', $v) AS s ORDER BY s DESC LIMIT k, served per store and merged when every type in play carries the store. Either way a cross-type ranking runs one search per relationship type (each store has its own HNSW index), so many small types cost about types × one search — with a long tail of types, the exact scan of each small store is often as fast as its index.</usage>\n");
    xml.push_str("    <caveat>A type in play without the store raises the scalar's own error — vector_score(): no embedding 'col_emb' found for relationship type 'X', or text_score(): no embedding for property 'col' on relationship type 'X' — so name only the types that carry it. Stores declaring different metrics refuse the procedure's merge unless metric is passed. Deleting an embedded relationship or an endpoint drops that store's HNSW index to none until build_index runs again, and a vacuum() that compacts drops every vector index in the graph, node and relationship alike; refresh_index refuses while there is none.</caveat>\n");
    xml.push_str("    <ex desc=\"cross-type procedure\">CALL db.relationship_embeddings.query({text_column:'description', text:'who founded it?', top_k:5}) YIELD relationship, score, type RETURN type, relationship.description, score</ex>\n");
...
pub(super) fn write_topic_ontology(xml: &mut String) {
    ...
    xml.push_str("    <summary>The declared semantic layer: an is_a forest over type names plus relationship semantics (domain/range, inverse naming, cardinality, required, transitive/ancestry/symmetric), persisted with the graph. Annotations, not axioms: it never changes what a query matches.</summary>\n");
    xml.push_str("    <usage>Declare with define_ontology({classes: {...}, relationships: {...}}). Inspect with SHOW ONTOLOGY or describe(). Audit with CALL ontology_audit() YIELD entity_kind, rule, severity, violations, exempted, total, pct, domain_class, property — entity_kind is node or edge; together with rule it identifies one scorecard row per declared check (both breakdown columns Null). CALL ontology_audit({by: 'domain_class'}) instead PARTITIONS each rule into one row per violating source class, with that class's violations/pct and the rule's own severity/exempted/total — the 'which source types are violating?' follow-up; the rows sum back to the rule's violations. CALL ontology_audit({by: 'property'}) is the 'which fields are missing?' follow-up: it CENSUSES the required_properties/property_types rules into one row per declared property, including properties nothing fails, and an edge missing several counts under each — so those rows sum to at least the aggregate, never back to it. One axis at a time; the column you did not ask for is Null. CALL edge_property_violation() YIELD relationship, check, source, target, property, properties, exempt lists the individual edges behind the required_properties/property_types counts (one row per flagged edge, properties listing every property it fails and property the first of them, exempted ones marked). The six rule procedures (type_domain_violation, type_range_violation, missing_required_edge, cardinality_violation, inverse_violation, transitivity_violation) called with NO arguments check every declaration; a domain/range naming an abstract class widens to its declared descendants. enforcement (advisory|warn|error, or a per-check map like {required_properties: error}) is acted on by blueprint builds: warn lands in the build report, error fails the build. Class required_properties/property_types cover primary-type nodes plus declared descendants; each ancestor contract applies independently, secondary labels do not enroll nodes, and aliases resolve on the actual node type. Node totals count covered live nodes, with zero exemptions. CALL node_property_violation() YIELD class, check, node, property, properties lists one row per violating node per declaring class/check (no parameters); properties lists every failed field. Relationship required_properties/property_types are audited per edge; inverse_name is naming-only unless inverse_enforced: true. exempt: {required_properties: ['SourceClass']} excuses violations whose edge source is that class (or a declared descendant): they are counted in the exempted column instead of against severity, so one legitimately-nonconforming source type cannot pin a rule at advisory. Accepted for required_properties and property_types only. transitive: true and ancestry: true are mutually exclusive and mean different things: transitive enrolls transitivity_violation, which requires a STORED closure (every a-&gt;b-&gt;c needs a stored a-&gt;c edge); ancestry is a documentation-only annotation that the chain is meaningful and is walked with *1.., which is what a parent-pointer taxonomy (STRAT_PARENT, wdt:P279) is — declaring that transitive reports 100% violations.</usage>\n");
    ...
    xml.push_str("    <caveat>After materialize_ontology(), label POSITION decides the plan: MATCH (n:Person) is the label engine (on a closed managed label whose every live member type is indexed for the filtered property it runs per-member index probes, shown as a ClosureProbe row in EXPLAIN), while WHERE n:Person is an ordinary post-candidate predicate — it binds every node and filters per row, and nothing rewrites it into a candidate set. Put the label in the pattern. MATCH (n:A|B) matches the union with nothing materialized, and never probes.</caveat>\n");
    xml.push_str("  </topic>\n");
}
