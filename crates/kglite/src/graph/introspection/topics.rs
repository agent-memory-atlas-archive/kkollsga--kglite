...
pub(super) fn write_cypher_overview(xml: &mut String, surface: DescribeSurface) {
    ...
    xml.push_str("    <proc name=\"db.node_embeddings.*\" yields=\"per procedure; query: node, score, search_method, type\">Node embedding stores through Cypher — the relationship procedures below with nodes in place of relationships, over the same stores set_embeddings / embed_texts / build_vector_index write. db.node_embeddings.set({type, text_property, entries:[{node:n, vector:[...]}], metric?}) YIELD stored, dimension upserts (add_embeddings' path); db.node_embeddings.embed({type | types:[...], text_property, nodes, mode?, batch_size?, metric?}) YIELD embedded, skipped, dimension, model embeds the listed nodes with the registered embedder; list, remove({type, text_property, nodes}), drop, build_index, refresh_index, drop_index take the relationship twins' parameters and yield their columns; db.node_embeddings.query({type | types:[...], text_property, vector:[...] | text:'query', top_k?, exact?, metric?}) YIELD node, score, search_method, type ranks whole stores merged into one top-k (stores declaring different metrics refuse the merge unless metric is passed). Writes roll back with a failing statement.</proc>\n");
    xml.push_str("    <proc name=\"db.node_text_index.*\" yields=\"build: indexed, skipped, terms; refresh: refreshed; drop: dropped; list: entity, type, property, documents, terms, skipped, index_state, delta, auto_refresh_limit\">The node BM25 index lifecycle in Cypher, over build_text_index's path: build({type, property, auto_refresh_limit?}), refresh({type, property}), drop({type, property}), list({type?, property?}). Score with text_bm25(n, 'property', 'query').</proc>\n");
    ...
    xml.push_str("    <proc name=\"db.relationship_embeddings.*\" yields=\"per procedure; query: relationship, score, search_method, type\">Relationship embedding stores, keyed by (relationship type, text property). db.relationship_embeddings.set({type, text_property, entries:[{relationship:r, vector:[...]}], metric?}) YIELD stored, dimension writes vectors (stored = vectors the store holds after the call, not the number written; set and embed refuse a text_property no relationship of the type carries); db.relationship_embeddings.embed({type | types:[...], text_property, relationships, mode?, batch_size?, metric?}) YIELD embedded, skipped, dimension, model generates them with the registered embedder (types:[...] runs one pass per listed type and yields one row: embedded and skipped summed, dimension and model shared or null); from Python, set_/add_relationship_embeddings and embed_relationship_texts are the bulk routes; db.relationship_embeddings.list({type?, text_property?}) YIELD entity, type, text_property, store, dimension, count, metric, model, index_state, delta, unembedded (delta = stored vectors the index does not hold, so delta equals count when no index is built); db.relationship_embeddings.remove({type, text_property, relationships}) and db.relationship_embeddings.drop({type, text_property}) delete vectors or the whole store; db.relationship_embeddings.build_index({type, text_property, m?, ef_construction?, ef_search?, metric?, auto_refresh_limit?}), db.relationship_embeddings.refresh_index({type, text_property}) and db.relationship_embeddings.drop_index({type, text_property}) manage the HNSW accelerator, which .kgl saves keep (refresh_index refuses when no index is built; SET and CREATE keep the index, while deleting an embedded relationship — DELETE r, or DETACH DELETE of either endpoint — drops it to index_state none until build_index runs again, as a node delete drops the node index; a vacuum() that compacts drops every vector index in the graph, node and relationship, including types that saw no delete); db.relationship_embeddings.query({type | types:[...], text_property, vector:[...] | text:'query', top_k?, exact?, metric?}) YIELD relationship, score, search_method, type ranks the whole store — or with types:[...], or with neither type nor types (every store for text_property), several stores merged into one top-k ordered by score, then type, then slot; stores declaring different metrics refuse the merge unless metric is passed (search_method is per row: hnsw only when that store's index served the query, else exact — exact:true, a stale index that cannot catch up, or a metric the index cannot serve all answer exact; text: is embedded once with the registered embedder). Inside MATCH, vector_score(r, 'col_emb', $v), text_score(r, 'col', ...) and embedding_norm(r, 'col_emb') score a bound relationship per row; ORDER BY vector_score(r, …) DESC LIMIT k is served from the store (HNSW when an index is online, {exact:true} forces exact) — see the relationship_semantic functions group.</proc>\n");
    xml.push_str("    <proc name=\"db.relationship_text_index.*\" yields=\"build: indexed, skipped, terms; refresh: refreshed; drop: dropped; list: entity, type, property, documents, terms, skipped, index_state, delta, auto_refresh_limit\">BM25 text indexes over one relationship type's property, the relationship twin of build_text_index. db.relationship_text_index.build({type, property, auto_refresh_limit?}) builds or replaces one; db.relationship_text_index.refresh({type, property}) folds in every change since; db.relationship_text_index.drop({type, property}) removes it (dropped false when absent); db.relationship_text_index.list({type?, property?}) reads freshness. Score with text_bm25(r, 'property', 'query'). Writes (SET, REMOVE, CREATE, MERGE) are folded in at the next query within auto_refresh_limit, a deleted relationship's document is removed at the delete, and a failed statement's build, drop or catch-up is undone. Saved in .kgl; memory and mapped storage only (disk refuses); not in the write-ahead log.</proc>\n");
    xml.push_str("  </procedures>\n");
...
pub(super) fn write_topic_node_semantic(xml: &mut String) {
    ...
    xml.push_str("    <ex desc=\"store procedure\">CALL db.node_embeddings.query({type:'Doc', text_property:'summary', text:'graph databases', top_k:5}) YIELD node, score RETURN node.title, score</ex>\n");
...
pub(super) fn write_topic_relationship_semantic(xml: &mut String) {
    ...
    xml.push_str("    <ex desc=\"cross-type procedure\">CALL db.relationship_embeddings.query({text_property:'description', text:'who founded it?', top_k:5}) YIELD relationship, score, type RETURN type, relationship.description, score</ex>\n");
