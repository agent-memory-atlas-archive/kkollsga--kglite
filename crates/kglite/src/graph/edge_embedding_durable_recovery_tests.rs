...
fn a_first_store_created_by_set_survives_a_crash() {
    ...
    assert_eq!(
        ...
        Some(StoreState {
            dimension: 2,
            // `db.edge_embeddings.set` with no `metric:` leaves the store's
...
fn dropping_a_store_inside_the_durable_window_survives_a_crash() {
    let fixture = Fixture::new(SEED, true);
    {
        ...
        run(
            &session,
            "CALL db.edge_embeddings.drop({type: 'CLAIMS', text_property: 'text'}) \
    ...
    let (expected_stores, expected_note) = {
        ...
        for step in 0..24u64 {
            match next() % 5 {
                ...
                _ => run(
                    &session,
                    "CALL db.edge_embeddings.drop({type: 'CLAIMS', text_property: 'note'}) \
                     YIELD dropped RETURN dropped",
                ),
            }
        }
