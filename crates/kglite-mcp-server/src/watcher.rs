...
/// The directory a watching mode registers with the OS, canonicalized. `None`
...
/// Start the filesystem watcher this mode wants, if any.
///
/// Only the producer-backed modes want one. `--graph` mode used to arm an
/// opt-in watch on the served file; it now stats that file on every tool call
/// instead (`tools::graph_reload`), which is unconditional, needs no OS watch
/// registration, and cannot miss a republish a watcher's filters swallowed.
