 ...
 *
 * <p>There is no {@code returning(node)} overload: use {@link #properties()}, {@link #labels()},
 * {@link #id()} or {@link #prop(String)}. The whole node is still reachable —
 * {@code ref().as("n")} emits {@code RETURN n AS n} and hands back the node as a structured
 * {@code Map} with {@code id}, {@code labels} and {@code properties}.
 */
public final class Node implements Pattern {
     ...
     * {@code count(n)}, {@code exists(n.prop)}. Aliased into a projection it returns the whole
     * node as a structured {@code Map} — see the class javadoc.
     *
