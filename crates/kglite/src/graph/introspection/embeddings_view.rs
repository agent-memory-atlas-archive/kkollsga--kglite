...
const RELATIONSHIP_SEMANTIC: &str = "relationships: vector_score(r, 'col_emb', $v) / text_score(r, 'col', 'query'|[...]) score a matched relationship, and embedding(r, 'col_emb') returns its stored vector (vector_score(r2, 'col_emb', embedding(r1, 'col_emb')) is edge-to-edge similarity) (ORDER BY … DESC LIMIT k is served from the store, through HNSW once indexed; {exact:true} forces exact); CALL db.edge_embeddings.query({type:'T' | types:['A','B'], text_property:'col', vector:$v | text:'query', top_k:10}) YIELD relationship, score, search_method, type ranks a whole store (HNSW once db.edge_embeddings.build_index has run; stores are per relationship type and text property — types:['A','B'], or neither type nor types, ranks several merged into one top-k, and MATCH ()-[r:A|B]->() … ORDER BY vector_score(r, …) DESC LIMIT k merges when every type carries the store; describe(cypher=['relationship_semantic']) has the details; deleting an embedded relationship or an endpoint drops that index to none until build_index runs again, and refresh_index refuses while there is none)";
