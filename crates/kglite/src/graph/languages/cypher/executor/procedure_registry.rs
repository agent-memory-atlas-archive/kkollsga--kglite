...
pub(super) const PROCEDURES: &[ProcedureSpec] = &[
    ...
    ProcedureSpec {
        ...
        aliases: &[],
        description: "Rule: nodes of {type} with no outgoing edge of {edge} (direction-validated); no-arg form checks every ontology declaration",
        columns: &["node", "rule"],
    },
    ProcedureSpec {
    ...
    ProcedureSpec {
        ...
        aliases: &[],
        description: "Rule: (a)-[rel_a]->(b) without a matching (b)-[rel_b]->(a); no-arg form checks every ontology declaration",
        columns: &["a", "b", "rule"],
    },
    ProcedureSpec {
        name: "transitivity_violation",
        aliases: &[],
        description: "Rule: (a)->(b)->(c) chains under {rel} where the direct (a)->(c) edge is absent; no-arg form checks every ontology declaration",
        columns: &["a", "b", "c", "rule"],
    },
    ProcedureSpec {
        name: "cardinality_violation",
        aliases: &[],
        description: "Rule: nodes of {type} whose outgoing-{edge} count is outside [min, max]; no-arg form checks every ontology declaration",
        columns: &["node", "count", "rule"],
    },
    ProcedureSpec {
        name: "type_domain_violation",
        aliases: &[],
        description: "Rule: edges of {edge} whose source node is not of {expected_source} type; no-arg form checks every ontology declaration (a declared class widens to its descendants)",
        columns: &["source", "target", "rule"],
    },
    ProcedureSpec {
        name: "type_range_violation",
        aliases: &[],
        description: "Rule: edges of {edge} whose target node is not of {expected_target} type; no-arg form checks every ontology declaration (a declared class widens to its descendants)",
        columns: &["source", "target", "rule"],
    },
    ProcedureSpec {
        name: "table.upsert",
