...
pub(super) fn write_topic_functions(xml: &mut String) {
    ...
    xml.push_str("    <group name=\"semantic\">text_score(n, 'col', 'query'|[0.1,0.2,...] [, metric] [, {exact:true}]) — similarity score; exact:true bypasses HNSW, otherwise a compatible top-k may use it; result diagnostics report actual_mode and fallback_reason (also with PROFILE), while EXPLAIN reports requested policy only; a list query is scored as your query vector, a string query is embedded via set_embedder() (metrics: 'cosine', 'poincare', 'dot_product', 'euclidean'); embedding_norm(n, 'col') — L2 norm of embedding vector (hierarchy depth in Poincaré space, 0=root, ~1=leaf)</group>\n");
    xml.push_str("    <group name=\"relationship_semantic\">vector_score(r, 'col_emb', $v), text_score(r, 'col', 'query'|[...] [, metric]) and embedding_norm(r, 'col_emb') score a bound relationship against its relationship embedding store (always exact). To rank a whole relationship store instead, CALL db.edge_embeddings.query({type:'T', text_property:'col', vector:$v | text:'query', top_k:10}) YIELD relationship, score, search_method — HNSW once db.edge_embeddings.build_index has run.</group>\n");
