...
pub(super) fn write_fluent_topic_vectors(xml: &mut String) {
    ...
    xml.push_str("      <m sig=\"build_vector_index(node_type, text_column, m=None, ef_construction=None, ef_search=None, metric=None, auto_refresh_limit=None)\">Build an HNSW index so search scales on large stores (opt-in; auto-used; exact=True bypasses). Later vector writes are folded in at query entry while the delta stays under auto_refresh_limit; deleting an embedded node drops it, and a vacuum() that compacts drops every vector index in the graph.</m>\n");
    xml.push_str("      <m sig=\"refresh_vector_index(node_type, text_column) / drop_vector_index / has_vector_index\">Fold outstanding vectors in now (refuses when no index is built — it never builds one), remove the index (the vectors stay), or probe one. SHOW INDEXES reports stale/delta/unembedded.</m>\n");
    xml.push_str("      <m sig=\"embeddings(node_type_or_text_column, text_column) / embedding(node_type, text_column, node_id) / relationship_embeddings(relationship_type, text_column, *, relationship_keys=None)\">Read stored vectors back: a node store by node id, or a relationship store as rows {source, target, source_type, target_type, key, vector} (an edge list plus edge features). In Cypher, embedding(x, 'col_emb') returns one node's or relationship's vector.</m>\n");
    xml.push_str("      <m sig=\"set_relationship_embeddings(relationship_type, text_column, embeddings, *, relationship_keys=None, metric=None) / add_relationship_embeddings(relationship_type, text_column, embeddings, *, relationship_keys=None, metric=None) / embed_relationship_texts(relationship_type, text_column, *, mode=None, batch_size=256, show_progress=True, metric=None)\">Bulk-write a relationship store: set_ replaces it and add_ upserts into it (as set_embeddings / add_embeddings for nodes), with vectors keyed by (source_id, target_id) — or (source_type, source_id, target_type, target_id), plus the key for a parallel-group member — or the rows relationship_embeddings returns; or embed every relationship of the type with the registered model, as embed_texts does for nodes. db.edge_embeddings.set / .embed are the in-query routes.</m>\n");
    xml.push_str("    </methods>\n");
    xml.push_str("    <examples>\n");
...
pub(super) fn write_cypher_overview(xml: &mut String, surface: DescribeSurface) {
    ...
    xml.push_str("    <proc name=\"db.edge_embeddings.*\" yields=\"per procedure; query: relationship, score, search_method, type\">Relationship embedding stores, keyed by (relationship type, text property). db.edge_embeddings.set({type, text_property, entries:[{relationship:r, vector:[...]}], metric?}) YIELD stored, dimension writes vectors (stored = vectors the store holds after the call, not the number written; set and embed refuse a text_property no relationship of the type carries); db.edge_embeddings.embed({type | types:[...], text_property, relationships, mode?, batch_size?, metric?}) YIELD embedded, skipped, dimension, model generates them with the registered embedder (types:[...] runs one pass per listed type and yields one row: embedded and skipped summed, dimension and model shared or null); from Python, set_/add_relationship_embeddings and embed_relationship_texts are the bulk routes; db.edge_embeddings.list({type?, text_property?}) YIELD entity, type, text_property, store, dimension, count, metric, model, index_state, delta, unembedded (delta = stored vectors the index does not hold, so delta equals count when no index is built); db.edge_embeddings.remove({type, text_property, relationships}) and db.edge_embeddings.drop({type, text_property}) delete vectors or the whole store; db.edge_embeddings.build_index({type, text_property, m?, ef_construction?, ef_search?, metric?, auto_refresh_limit?}), db.edge_embeddings.refresh_index({type, text_property}) and db.edge_embeddings.drop_index({type, text_property}) manage the HNSW accelerator, which .kgl saves keep (refresh_index refuses when no index is built; SET and CREATE keep the index, while deleting an embedded relationship — DELETE r, or DETACH DELETE of either endpoint — drops it to index_state none until build_index runs again, as a node delete drops the node index; a vacuum() that compacts drops every vector index in the graph, node and relationship, including types that saw no delete); db.edge_embeddings.query({type | types:[...], text_property, vector:[...] | text:'query', top_k?, exact?, metric?}) YIELD relationship, score, search_method, type ranks the whole store — or with types:[...], or with neither type nor types (every store for text_property), several stores merged into one top-k ordered by score, then type, then slot; stores declaring different metrics refuse the merge unless metric is passed (search_method is per row: hnsw only when that store's index served the query, else exact — exact:true, a stale index that cannot catch up, or a metric the index cannot serve all answer exact; text: is embedded once with the registered embedder). Inside MATCH, vector_score(r, 'col_emb', $v), text_score(r, 'col', ...) and embedding_norm(r, 'col_emb') score a bound relationship per row; ORDER BY vector_score(r, …) DESC LIMIT k is served from the store (HNSW when an index is online, {exact:true} forces exact) — see the relationship_semantic functions group.</proc>\n");
    xml.push_str("    <proc name=\"db.edge_text_index.*\" yields=\"build: indexed, skipped, terms; refresh: refreshed; drop: dropped; list: entity, type, property, documents, terms, skipped, index_state, delta, auto_refresh_limit\">BM25 text indexes over one relationship type's property, the relationship twin of build_text_index. db.edge_text_index.build({type, property, auto_refresh_limit?}) builds or replaces one; db.edge_text_index.refresh({type, property}) folds in every change since; db.edge_text_index.drop({type, property}) removes it (dropped false when absent); db.edge_text_index.list({type?, property?}) reads freshness. Score with text_bm25(r, 'property', 'query'). Writes (SET, REMOVE, CREATE, MERGE) are folded in at the next query within auto_refresh_limit, a deleted relationship's document is removed at the delete, and a failed statement's build, drop or catch-up is undone. Saved in .kgl; memory and mapped storage only (disk refuses); not in the write-ahead log.</proc>\n");
    xml.push_str("  </procedures>\n");
...
pub(super) fn write_topic_relationship_semantic(xml: &mut String) {
    ...
    xml.push_str("    <usage>An embedding store belongs to one relationship type and one text property (describe() shows it on that type's conn line as embeddings=\"col(dim=D,count=N)\", with ,hnsw once an index is built). Write vectors with db.edge_embeddings.set or .embed in a query, or in bulk from Python with set_/add_relationship_embeddings / embed_relationship_texts; index with db.edge_embeddings.build_index. To rank across relationship types, either CALL db.edge_embeddings.query({types:['A','B'], text_property:'col', ...}) — or leave out type and types to rank every store for text_property — which merges the stores into one top-k ordered by score, then type, then slot (each row yields type and its own search_method); or MATCH ()-[r:A|B]->() (or untyped ()-[r]->()) RETURN r, vector_score(r, 'col_emb', $v) AS s ORDER BY s DESC LIMIT k, served per store and merged when every type in play carries the store.</usage>\n");
