...
pub(super) fn write_cypher_overview(xml: &mut String, surface: DescribeSurface) {
    ...
    xml.push_str("    <clause name=\"SHOW INDEXES\">List installed indexes as a read: columns name, type (PROPERTY|RANGE|FULLTEXT), entityType, labelsOrTypes, properties, state, stale, delta. stale/delta are null except on FULLTEXT (BM25) rows, where they report whether the index is behind the graph and by how many documents. Same rows as CALL db.indexes(); use that form when you need YIELD or WHERE.</clause>\n");
