...
pub(super) fn write_fluent_topic_vectors(xml: &mut String) {
    ...
    xml.push_str("      <m sig=\"set_relationship_embeddings(relationship_type, text_column, embeddings, *, relationship_keys=None, metric=None) / add_relationship_embeddings(same) / embed_relationship_texts(relationship_type, text_column, *, mode=None, batch_size=256, show_progress=True, metric=None)\">Bulk-write a relationship store: set_ replaces it and add_ upserts into it (as set_embeddings / add_embeddings for nodes), with vectors keyed by (source_id, target_id) — or (source_type, source_id, target_type, target_id), plus the key for a parallel-group member — or the rows relationship_embeddings returns; or embed every relationship of the type with the registered model, as embed_texts does for nodes. db.edge_embeddings.set / .embed are the in-query routes.</m>\n");
    xml.push_str("    </methods>\n");
    xml.push_str("    <examples>\n");
