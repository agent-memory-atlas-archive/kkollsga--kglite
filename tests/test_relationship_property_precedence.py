...
def test_knwler_relation_id_round_trips_through_yield_relationship():
    ...
    graph.cypher(
        ...
        "CALL db.relationship_embeddings.embed({type:'RELATION', text_column:'description', relationships: rs}) "
        "YIELD embedded RETURN embedded"
    )
    got = rows(
        graph,
        "CALL db.relationship_embeddings.query({type:'RELATION', text_column:'description', text:'alpha', top_k:1}) "
